      ...
      <div class="row g-4">
        <div class="col-md-4" v-for="(agent, index) in filteredAgents" :key="index">
          <div class="agent-card-custom">
            <img :src="'/storage/' + agent.image" :alt="agent.image" class="robot-img-custom pulse-animation" />
            <div class="agent-title-custom">{{ agent.name }}</div>
            <div class="agent-role-custom">{{ agent.domain }}</div>
            <div class="agent-desc-custom">
              {{ agent.description }}
            </div>
      <a :href="`/detailsAgent/${agent.id}`" class="voir-plus-btn-custom">Voir plus</a>
          </div>
        </div>
      </div>
