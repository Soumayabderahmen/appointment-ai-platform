                                ...
                                <div v-if="showModal" class="modal-overlay">
                                    <div class="modal-content">
                                        ...
                                        <form @submit.prevent="submitForm">
                                            ...
                                            >
                                                <div
                                                    class="d-flex justify-between align-items-center mb-2"
                                                >
                                                    <label class="mb-0"
                                                        >Sous-titre</label
                                                    >
                ...
                <div class="card-body">
                    <ul class="timeline mb-0">
                        ...
                        >
                            ...
                            ></span>
                            <div
                                class="timeline-event p-3"
                                style="margin-left: -24px"
                            >
                                <div
                                    class="timeline-heet on the flightader mb-3"
                                >
                                    <h6 class="mb-0">
                                ...
                                <div class="content">
                                    <ul>
                                        <li
                                            v-for="(
                                                task, taskIndex
                                            ) in responsibility.tasks"
                                            :key="taskIndex"
                                        >
                                            <img
                                                src="/assets/img/dash/fleche.png"
                                                alt=""
                                            />
                                            {{ task }}
                                        </li>
...
const openEditModal = () => {
    form.value = {
        ...
        image: null,
        sections:
            props.agent.sections?.map((s) => ({
                id: s.id,
                title: s.title,
                tasks:
                    s.tasks?.map((t) => ({ id: t.id, content: t.content })) ||
                    [],
            })) || [],
    };
...
const submitForm = async () => {
    try {
        ...
        await axios.post(
            `/api/admin/agent/${props.agent.id}/update`,
            formData,
            {
                headers: {
        ...
        toast.error(
            "Une erreur est survenue lors de la mise à jour de l’agent."
        );
    }
};
...
responsibilities.value =
    agent.value.sections?.map((section) => ({
        title: section.title,
...
const fetchAgent = async () => {
    try {
        const res = await axios.get(`/api/admin/agent/${props.agent.id}`);
