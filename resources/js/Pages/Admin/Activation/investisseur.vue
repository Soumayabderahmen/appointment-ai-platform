...
const search = ref("");
const localInvestisseur = ref([...props.investisseurs]);
...
const filteredinvestisseurs = computed(() => {
    return localInvestisseur.value.filter((r) => {
        const queryMatch =
            !search.value ||
...
const totalPages = computed(() =>
    Math.ceil(filteredinvestisseurs.value.length / perPage)
);
...
const activateInvestisseur = (investisseurId) => {
    axios.post(`/api/admin/activation/${investisseurId}/investisseur`)
        .then(response => {
           
            console.log(response.data);

           
            const investisseurIndex = localInvestisseur.value.findIndex(investisseur => investisseur.id == investisseurId);
            console.log('startupIndex:', investisseurIndex);

            if (investisseurIndex !== -1) {
                localInvestisseur.value[investisseurIndex].statut = 'active';
            }

            toast.success(response.data.message || 'Investisseur activated successfully');
        })
        .catch(error => {
            // Handle error response
            console.error(error);
            toast.error(error.response?.data?.message || 'Failed to activate Investisseur');
        });
};
</script>
...
<template>
    <div class="px-6 pb-4 flex justify-between items-center">
            ...
            <div class="d-lg-flex d-md-flex d-sm-flex d-block">
                ...
                >
                    ...
                    <option
                        v-for="r in [
                            ...new Set(
                                props.investisseurs.map((res) => res.statut)
                            ),
                        ]"
                        :key="r"
                        :value="r"
        ...
        <span class="text-sm text-gray-500 ml-4"
            >{{ filteredinvestisseurs.length }} résultat(s)</span
        >
    </div>
    <div class="card-body">
        <div class="flex flex-col gap-4 mt-4">
            <!-- En-têtes -->
            <div
                class="flex items-center px-6 py-3 font-semibold text-sm rounded-[12px]"
            >
...
"            >
            <div
            class="w-10 h-6 rounded-full bg-gray-100 flex items-center justify-center text-lg font-bold text-gray-700">
            {{ investisseur.name.charAt(0).toUpperCase() }}
          </div>
                <!-- Nom -->
                ...
                <div class="w-1/5 text-center text-[#0093EE]">
                    {{
                        investisseur.visibility === "public"
                            ? "🔓 Public"
                            : "🔐 Private"
                    }}
                </div>
            ...
            <!-- Aucun résultat -->
            <div
                v-if="paginatedinvestisseurs.length === 0"
                class="bg-white rounded-xl p-6 text-center shadow"
            >
