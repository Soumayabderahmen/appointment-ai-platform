...
const search = ref("");
const localStartups = ref([...props.startupes]);
...
const filteredinvestisseurs = computed(() => {
    return localStartups.value.filter((r) => {
        const queryMatch =
            !search.value ||
...
const activateStartup = (startupId) => {
    axios.post(`/api/admin/activation/${startupId}/startup`)
        .then(response => {
           
            console.log(response.data);

           
            const startupIndex = localStartups.value.findIndex(startup => startup.id == startupId);
            console.log('startupIndex:', startupIndex);

            if (startupIndex !== -1) {
                localStartups.value[startupIndex].statut = 'active';
            }

            toast.success(response.data.message || 'Startup activated successfully');
        })
        .catch(error => {
            // Handle error response
            console.error(error);
            toast.error(error.response?.data?.message || 'Failed to activate startup');
        });
};
</script>
            ...
            <div class="d-lg-flex d-md-flex d-sm-flex d-block">
                ...
                >
                    ...
                    <option
                        v-for="r in [
                            ...new Set(
                                props.startupes.map((res) => res.statut)
                            ),
                        ]"
                        :key="r"
                        :value="r"
